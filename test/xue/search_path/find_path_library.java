...
public class find_path_library {
    ...
    find_path_library(File Shape, File inFile1, File inFile2, File out) throws Exception {
    ...
    public static LinkedList<String> search_origin_path(double start_lat, double start_lon, double end_lat, double end_lon, File shape) throws Exception {
        ...
        Point endPoint = gf.createPoint(new Coordinate(end_lon,end_lat));
        ...
        if(p!=null){
            ...
            point = gf.createPoint(new Coordinate(((Point) current.getObject()).getY(), ((Point) current.getObject()).getX()));
